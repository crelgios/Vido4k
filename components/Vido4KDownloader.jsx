...
const TEST_CASES = [
  ...
  { name: "YouTube is blocked", input: "https://www.youtube.com/watch?v=abc123", expectedType: "platform" },
  { name: "mobile YouTube is blocked", input: "https://m.youtube.com/watch?v=abc123", expectedType: "platform" },
  { name: "short YouTube is blocked", input: "https://youtu.be/abc123", expectedType: "platform" },
  { name: "Instagram is blocked", input: "https://www.instagram.com/reel/abc123/", expectedType: "platform" },
  { name: "Facebook is blocked", input: "https://facebook.com/watch/?v=123", expectedType: "platform" },
...
export default function Vido4KDownloader() {
  ...
  const [fileName, setFileName] = useState("vido4k-video.mp4");
  ...
  return (
    <main className="min-h-screen bg-slate-950 text-white">
        ...
        <div className="relative mx-auto grid max-w-6xl items-center gap-10 md:grid-cols-2">
          <div>
            <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-emerald-400/20 bg-emerald-400/10 px-4 py-2 text-sm text-emerald-200">
              ...
              Fast direct 4K video file downloader
            </div>
            ...
            <p className="mt-5 max-w-xl text-base leading-7 text-slate-300 md:text-lg">
              Vido4K lets you check and download direct video file links in your browser. Use it for MP4, WebM, MOV, M4V and OGG files that you own or have permission to download.
            </p>
          ...
          <form onSubmit={handleSubmit} className="rounded-3xl border border-white/10 bg-white/10 p-4 shadow-2xl shadow-glow backdrop-blur-xl md:p-6">
            <div className="rounded-3xl bg-slate-950 p-5 md:p-6">
              <div className="mb-5 flex items-center justify-between gap-3">
                <div>
                  <h2 className="text-xl font-bold">Paste video link</h2>
                  <p className="mt-1 text-sm text-slate-400">Check and prepare a direct file download</p>
                </div>
              ...
              <p className="mt-4 text-xs leading-5 text-slate-500">
                Vido4K is for direct file links only. It does not bypass platform restrictions or download private/protected videos.
              </p>
            </div>
          </form>
        </div>
      ...
      <section id="features" className="mx-auto grid max-w-6xl gap-4 px-5 py-8 md:grid-cols-3">
        <FeatureCard icon="shield" title="Safe link checks" text="Vido4K checks whether the link is a direct video file or an unsupported protected platform URL." />
        <FeatureCard icon="globe" title="Browser based" text="Direct video links download through the browser. Videos are not uploaded to or stored on this website." />
        <FeatureCard icon="file" title="Supported files" text="Use direct MP4, WebM, MOV, M4V and OGG video file URLs with 4K, 2K and HD labels." />
      </section>

      <section id="how-it-works" className="mx-auto max-w-6xl px-5 py-10">
        <div className="rounded-3xl border border-white/10 bg-white/5 p-6 md:p-8">
          <p className="text-sm font-bold uppercase tracking-[0.25em] text-emerald-300">How it works</p>
          <h2 className="mt-3 text-2xl font-black md:text-3xl">Download direct video files in four steps</h2>
          <div className="mt-6 grid gap-4 md:grid-cols-4">
            <Step number="1" title="Paste link" text="Add a direct video file URL in the input box." />
            <Step number="2" title="Check video" text="The site validates the URL format and file type." />
            <Step number="3" title="Choose label" text="Select 4K, 2K, Full HD, HD or original file label." />
            <Step number="4" title="Download" text="Get a browser download button or a clear error message." />
          </div>
        </div>
      </section>

      <section id="supported-files" className="mx-auto max-w-6xl px-5 py-10">
        <div className="grid gap-5 md:grid-cols-2">
          <div className="rounded-3xl border border-white/10 bg-white/5 p-6 md:p-8">
            <h2 className="text-2xl font-bold">Supported direct video formats</h2>
            <p className="mt-4 leading-7 text-slate-300">
              Vido4K works with direct links ending in MP4, WebM, MOV, M4V and OGG. These are common video file formats used for websites, cloud storage exports, learning files, product videos and personal media libraries.
            </p>
          </div>
          <div className="rounded-3xl border border-white/10 bg-white/5 p-6 md:p-8">
            <h2 className="text-2xl font-bold">Built for legal file downloads</h2>
            <p className="mt-4 leading-7 text-slate-300">
              This tool is not made to bypass restrictions. It is designed for direct video files where you have permission, such as your own videos, public test files, business media, course files or hosted video assets.
            </p>
          </div>
        </div>
      </section>
      ...
      <section id="faq" className="mx-auto max-w-6xl px-5 py-10">
        <p className="text-sm font-bold uppercase tracking-[0.25em] text-emerald-300">FAQ</p>
        <h2 className="mt-3 text-2xl font-black md:text-3xl">Vido4K questions</h2>
        <div className="mt-5 grid gap-4 md:grid-cols-2">
          <Faq q="Can Vido4K download protected platform videos?" a="No. Vido4K checks unsupported links and shows an error because this site does not bypass platform restrictions." />
          ...
          <Faq q="Can I choose 4K?" a="Yes. The tool includes 4K 2160p, 2K 1440p, Full HD 1080p, HD 720p, Mobile 480p and Original file labels." />
          <Faq q="Will videos be stored on my server?" a="No. This design uses browser downloads for direct file links and does not store user videos." />
        </div>
      </section>
      ...
      <footer className="border-t border-white/10 px-5 py-8 text-center text-sm text-slate-400">
        © 2026 Vido4K. Built for direct 4K video file downloads.
      </footer>
    </main>
  );
}
...
function validateUrl(value) {
  ...
  if (isBlocked) {
    return {
      ok: false,
      type: "platform",
      message: "This looks like a protected platform link. Vido4K cannot download protected platform videos. Use official download options or direct video files you own."
    };
  }
...
function normalizeFileName(value) {
  ...
  const finalName = cleaned || "vido4k-video.mp4";
  const lowerName = finalName.toLowerCase();
  const hasExtension = VIDEO_EXTENSIONS.some((extension) => lowerName.endsWith(`.${extension}`));
      ...
      <div className="mx-auto flex max-w-6xl items-center justify-between gap-4">
        <a href="/" className="flex items-center gap-3" aria-label="Vido4K home">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400">Direct 4K downloader</p>
          </div>
        </a>
        ...
        <nav className="hidden items-center gap-6 text-sm text-slate-300 md:flex" aria-label="Main navigation">
          ...
          <a href="#supported-files" className="hover:text-white">Formats</a>
...
function Step({ number, title, text }) {
  return (
    <article className="rounded-3xl bg-slate-900 p-5">
      ...
      <div className="rounded-3xl border border-amber-400/20 bg-amber-400/10 p-5 text-sm text-amber-100">
        ...
        <ul className="mt-3 space-y-2">
          {failedTests.map((test) => (
            <li key={test.name}>{test.name}: expected {test.expectedType}, got {test.actualType}</li>
          ))}
        </ul>
      </div>
    </section>
...
function Icon({ name, className = "h-5 w-5" }) {
  ...
  if (name === "link") {
    return <svg {...commonProps}><path d="M10 13a5 5 0 0 0 7 0l2-2a5 5 0 0 0-7-7l-1 1" /><path d="M14 11a5 5 0 0 0-7 0l-2 2a5 5 0 0 0 7 7l1-1" /></svg>;
  }

  if (name === "sparkles") {
    return <svg {...commonProps}><path d="m12 3 2 5 5 1-5 2-2 5-2-5-5-2 5-1 2-5Z" /><path d="M19 14l1 2 2 1-2 1-1 2-1-2-2-1 2-1 1-2Z" /></svg>;
  }

  if (name === "loader") {
    return <svg {...commonProps}><path d="M21 12a9 9 0 1 1-6-8" /></svg>;
  }

  if (name === "play") {
    return <svg {...commonProps}><path d="M8 5v14l11-7-11-7Z" /></svg>;
  }

  if (name === "check") {
    return <svg {...commonProps}><circle cx="12" cy="12" r="10" /><path d="m8 12 3 3 5-6" /></svg>;
  }

  if (name === "alert") {
    return <svg {...commonProps}><path d="M10 4 2 19a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2L14 4a2 2 0 0 0-4 0Z" /><path d="M12 9v4" /><path d="M12 17h.01" /></svg>;
  }
  ...
  if (name === "globe") {
    return <svg {...commonProps}><circle cx="12" cy="12" r="10" /><path d="M2 12h20" /><path d="M12 2a15 15 0 0 1 0 20" /><path d="M12 2a15 15 0 0 0 0 20" /></svg>;
  }

  if (name === "file") {
    return <svg {...commonProps}><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8Z" /><path d="M14 2v6h6" /><path d="m10 13 5 3-5 3v-6Z" /></svg>;
  }
