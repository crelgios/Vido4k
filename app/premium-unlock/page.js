...
const ACCESS_TESTS = [
  { name: "category list has movies", actual: CATEGORIES.includes("Movies"), expected: true },
  { name: "category list has websites", actual: CATEGORIES.includes("Websites"), expected: true },
  { name: "movies category has website choices", actual: getWebsiteOptions("Movies").length > 0, expected: true },
  { name: "websites category has website choices", actual: getWebsiteOptions("Websites").length > 0, expected: true },
  { name: "first movie website is selected by default", actual: getDefaultWebsite("Movies"), expected: "Movie Hub" },
  { name: "fallback website option works", actual: getDefaultWebsite("Unknown"), expected: "Vido4K" }
];
...
function loadNativeAd() {
  ...
  const existingScript = document.getElementById("adsterra-native-script-premium");
  if (existingScript) return;

  const script = document.createElement("script");
  script.id = "adsterra-native-script-premium";
...
function runAccessTests() {
  return ACCESS_TESTS.map((test) => ({ ...test, passed: test.actual === test.expected })).filter((test) => !test.passed);
}
    ...
    <section className="mx-auto max-w-6xl px-5 py-6">
      <div className="rounded-3xl border border-amber-400/20 bg-amber-400/10 p-5 text-sm text-amber-100">
        ...
        <ul className="mt-3 space-y-2">
          {failedTests.map((test) => (<li key={test.name}>{test.name}</li>))}
        </ul>
      </div>
    </section>
  );
...
function Icon({ name, className = "h-5 w-5" }) {
  ...
  if (name === "key") return <svg {...commonProps}><circle cx="7.5" cy="15.5" r="5.5" /><path d="m12 11 8-8" /><path d="m17 6 3 3" /><path d="m14 9 3 3" /></svg>;
  if (name === "lock") return <svg {...commonProps}><rect x="4" y="11" width="16" height="10" rx="2" /><path d="M8 11V7a4 4 0 0 1 8 0v4" /></svg>;
  ...
  return <svg {...commonProps}><path d="M12 2 4 6v6c0 5 8 10 8 10s8-5 8-10V6l-8-4Z" /></svg>;
}
